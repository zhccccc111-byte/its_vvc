...
| in_mem/tp_buf 改为同步读 | 释放 ~2000 个分布式 RAM LUT | 中（需改引擎接口） |
...
**当前状态：** 功能正确，108 个测试用例全部通过。资源利用率低（~5% LUT）。时序在 100MHz 约束下轻微违例（-0.421ns），实际最高频率 ~96MHz。功耗 0.219W。目标器件 Artix-7 xc7a200tfbg484-3。
