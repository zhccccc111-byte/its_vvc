...
| 指标 | 值 | 说明 |
|------|-----|------|
| WNS (Setup) | **-5.378ns** | 违例 |
| TNS (Setup) | -92,228.6ns | 36,296 个违例端点 |
| WHS (Hold) | -0.230ns | 轻微违例 |
| WPWS (Pulse Width) | -0.234ns | 轻微违例 |
| 实际内部极限频率 | **~135MHz** | = 1/(2.0+5.378) ns |
...
| 维度 | 结果 |
|------|------|
| 内部逻辑极限 (FPGA Artix-7 -3) | **~135MHz** |
| 500MHz 差距 | 5.378ns (需减少 ~2.7ns) |
...
3. 最后做 P2（递推计数器），冲击 500MHz
4. 每步后跑 OOC 综合验证 WNS 改善
